...
public class AudioManager : MonoBehaviour
{
    ...
    {
        return _source0.volume;
    }

    public void UpdateVolume(float volume)
    {
        _source0.volume = volume;
        _source1.volume = volume;
    }

    public void LoadAudio()
    {
        Debug.Log("Loading Audio", gameObject);
        foreach (Audio clip in Audios)
        {
            // Debug.Log("Loading " + clip.name , gameObject);
            ListOfAudioClips.Add(clip.Name, clip.clip);
        }
    }
    public float GetSoundSFXLength(string name)
    {
        AudioClip clip;
        if (ListOfAudioClips.TryGetValue(name, out clip))
        {
            return clip.length;
        }
        return 0;
    ...
    public void PlaySoundSFX(AudioSource source, string name)
    {
        if (string.IsNullOrEmpty(name))
        {
            return;
    ...
    public void SetMusic(string name)
    {
        ...
        if (ListOfAudioClips.TryGetValue(name, out clip))
        {
            if (_source0.clip == clip && _source1.clip == clip)
            {
                return;
            }
            CrossFade(clip, 1, 1, 0);
        }
        else
        {
            Debug.Log(name + " not found");
